  ...
  .then(() => {
    ...
    if (user) {
      ...
      user.getIdToken().then(async (idToken) => {
        token = idToken;
        await printPacienteList(token);
        await printPacienteAnterioresList(token);
      }).catch((error) => {
...
contenidoPaciente.addEventListener("click", e => {
  ...
  if (e.target.closest('.editar')) {
    ...
    // console.log(e.target.parentElement.id);
    // e.target.parentElement.remove();
    window.location.pathname = `/consultaSecretario/${idProduct}`;
  }
    ...
    const listPaciente = pacientes.message
      .filter(paciente => {
        ...
        .map(pacientes => {
          ...
          const pagoRealizado = pacientes.paciente.pagoNoRealizado;

          let pagos = '';

          if (pagoRealizado) {
            pagos = `
            <span id="pagos" class="icon icon-paper pagos" style="visibility: hidden;"><ion-icon name="card"></ion-icon></span>
          `
          } else {
            pagos = `
            <span id="pagos" class="icon icon-paper pagos"><ion-icon name="card"></ion-icon></span>
          `
          }
...
contenidoPacienteAnteriores.addEventListener("click", e => {
  ...
  if (e.target.closest('.editar')) {
    ...
    // console.log(e.target.parentElement.id);
    // e.target.parentElement.remove();
    window.location.pathname = `/pacienteSecretario/${idProduct}`;
  } 
})
  ...
  printPacienteList()
    .then((pacientes) => {
      ...
      const listPaciente = filteredOnlyPacientes.map(paciente => {
        ...
        const pagoRealizado = paciente.paciente.pagoNoRealizado;
